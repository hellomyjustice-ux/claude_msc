...
export default async function MyPage() {
  ...
  const enrichedPosts: PostWithDetails[] = (posts ?? []).map((post) => ({
    ...post,
    likes_count: post.post_likes?.[0]?.count ?? 0,
    user_has_liked: false,
  }));

  const totalLikes = enrichedPosts.reduce((sum, p) => sum + p.likes_count, 0);
        ...
        <div className="grid sm:grid-cols-[160px_1fr] gap-8 items-start">
          ...
          <div className="flex flex-col gap-6">
            <div>
              ...
              {profile.bio && (
                <p className="text-sm text-[#3D5060] leading-relaxed max-w-md">{profile.bio}</p>
              )}
            </div>
            ...
            <div className="flex gap-8">
              <div>
                <p className="font-display text-3xl text-[#2B3A4E]">{enrichedPosts.length}</p>
            ...
            <p className="text-xs text-[#7A8E9C]">{user.email}</p>
          </div>
        </div>
      </section>
