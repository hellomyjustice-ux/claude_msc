...
import { clientCreatePost, clientUpdatePost } from '@/lib/supabase/client-actions';
...
interface ExistingImage extends PostImage { markedForDelete?: boolean; }
...
export default function PostForm({ mode, post }: PostFormProps) {
  ...
  const [existingImages, setExistingImages] = useState<ExistingImage[]>(post?.post_images ?? []);
  ...
  const toggleDeleteExisting = (id: string) => {
    setExistingImages((prev) => prev.map((img) => img.id === id ? { ...img, markedForDelete: !img.markedForDelete } : img));
  };

  const onSubmit = async (data: PostFormData) => {
    setServerError('');
    if (newImages.some((img) => img.error)) {
      setServerError('업로드할 수 없는 이미지가 있습니다.');
      return;
    }

    if (mode === 'create') {
      const result = await clientCreatePost({
      ...
      router.push(`/posts?id=${result.postId}`);
    } else if (post) {
      const toDelete = existingImages.filter((img) => img.markedForDelete);
      const result = await clientUpdatePost(post.id, {
        title: data.title, content: data.content, category: data.category,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-10">
      ...
      <Input label="제목" placeholder="제목을 입력하세요" error={errors.title?.message}
        className="text-2xl font-display py-3" {...register('title')} />
            ...
            <div className="grid grid-cols-3 gap-3">
              {existingImages.map((img) => (
                <div key={img.id} className="relative group">
                  <div
                    className={`relative aspect-square overflow-hidden bg-[#D4C49A] cursor-pointer ${img.markedForDelete ? 'opacity-40 ring-2 ring-[#9C4038]' : ''}`}
      ...
      <Textarea label="본문" placeholder="이야기를 기록해주세요" rows={12}
        error={errors.content?.message} {...register('content')} />
